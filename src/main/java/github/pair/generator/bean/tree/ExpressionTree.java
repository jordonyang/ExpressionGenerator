...
public class ExpressionTree {
     ...
     */
    public ExpressionTree(int operatorCount,int range) {
     ...
     * @param origin  待处理结点
     */
    private void insertNumbers(Node origin) {
        if (origin != null) {
            if (origin.getLeft() != null) {
                if (origin.getRight() != null) {
                    // 左右均不空，继续往两边子树递归
                    insertNumbers(origin.getLeft());
                    insertNumbers(origin.getRight());
                }else {
                    // 右空左不空， 插右，继续往左子树递归
                    origin.setRight(new Node<>(RandomUtil.generateFraction(range)));
                    origin.getRight().setParent(origin);
                    insertNumbers(origin.getLeft());
                }
            }else {
                // 左空右不空， 插左，继续往右子树递归
                if (origin.getRight() == null) {
                    origin.setLeft(new Node<>(RandomUtil.generateFraction(range)));
                    origin.getLeft().setParent(origin);
...
//    }
    ...
    private TraverseResult inorderTraverse(Node node) {
        ...
        if (!node.hasChild()) {
            // 数据结点
            return new TraverseResult(node.getData().toString(),
                                (Fraction) node.getData());
        }
        ...
        Fraction resultData = RandomUtil.symbolMatcher.get(node.getData().toString()).
                getCalculator().calculate(leftResult.getData(), rightResult.getData());

        String rawExpression = leftResult.getSubExpression() + " " + node.getData()
                + " " + rightResult.getSubExpression();

        /*
         * 如果当前结点操作符为'-'，且得到的左右子表达式计算结果为负数，
         * 则对该计算结果取绝对值, 同时将左右表达式位置交换，
         * 对于原二叉树中的数据结构则不作处理
         */
        if (node.getData() == Symbol.SUBTRACT && resultData.getNumerator() < 0) {
            resultData = new Fraction(Math.abs(resultData.getNumerator()),
                                            resultData.getDenominator());
        ...
        if (parent != null && ((Symbol)parent.getData()).
                        priGreaterThan((Symbol)node.getData())) {
            finalResult.setSubExpression(Symbol.LEFT_BRACKET +
                            rawExpression + Symbol.RIGHT_BRACKET);
        }else {
...
//     * @return 遍历结果
//     */
//    public TraverseResult getResult(Node node) {
